...
A production-ready finance application with a Go backend and TypeScript frontend. Built with modern best practices, clean architecture, and comprehensive tooling.

## Features

- **Monorepo Structure**: Organized for efficient builds and development
- **Go Backend**: High-performance REST API
...
```bash
git clone https://github.com/omar-shahieen/moneyflow.git
cd moneyflow
```
...
The backend uses environment variables prefixed with `MONEYFLOW_`. Key variables include:

- `MONEYFLOW_DATABASE_*` - PostgreSQL connection settings
- `MONEYFLOW_SERVER_*` - Server configuration
- `MONEYFLOW_AUTH_*` - Authentication settings
- `MONEYFLOW_REDIS_*` - Redis connection
- `MONEYFLOW_EMAIL_*` - Email service configuration
- `MONEYFLOW_OBSERVABILITY_*` - Monitoring settings

See `apps/backend/.env.example` for a complete list.

## Architecture

MoneyFlow follows clean architecture principles:
